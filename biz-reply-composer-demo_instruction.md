...
開発機の実測スペックを設計上の基準値とする（「8GB RAM機」のような一般論の想定は使わない）：総RAM 7.4GB、OS・ブラウザ・常駐アプリ稼働中の実効空きメモリは約800MB〜1.7GB、GPU/VRAMなしのCPU推論、モデル格納先は外付けUSB SSD（Fドライブ）。

- 既定：Gemma 3 1B（Google製。上記の実効空きメモリでロード・生成が現実的に完走する唯一帯。1コールドロード込みで約20秒応答）
- 高品質代替：Llama-3-ELYZA-JP-8B（日本ELYZA製・Metaベース。日本語特化。メモリ8GB以上の環境向け。本機では非現実的）
